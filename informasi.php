...
<html lang="en">
  ...
  <body>
    ...
    <div class="bg-primary text-white" style="min-height: 50px">
      <div class="container">
        <h1 class="text-center lh-base text-truncate">Informasi Kecamatan Weleri</h1>
      </div>
    </div>
    <!-- Title akhir -->

    <!-- Informasi -->
    <div class="informasi" style="font-family: Poppins;">
      <div class="container">
        <div class="row justify-content-center mt-5">
          <?php
            ...
            $jmlInformasi = mysqli_num_rows($hasil);
            if ($jmlInformasi > 0){
              while($row = mysqli_fetch_assoc($hasil))
            {
              $infor = $informasi;
          ?>
            <div class="col-lg-3 flex-wrap mb-2">
              <div class="card border-success shadow" data-aos="flip-right" data-aos-duration="3000">
                <p class="card-text fw-bold text-center mt-2 text-truncate"><?= $row['judul_informasi']; ?></p>
                <img src="dashboard/image/<?php echo $row['foto']; ?>" class="card-img-top" alt="Informasi">
                <div class="card-body">
                  <p class="card-text text-dark"><span><?= $row['hari']; ?> <?php echo date('d-m-Y', strtotime($row['tanggal'])); ?></p>
                </div>
                <div class="card-footer">
                <button class="btn btn-success" data-bs-toggle="modal" href="#Informasi<?=$informasi++?>">Selengkapnya</button>
                </div>
              </div>
                <!-- Modal -->
                <div class="modal fade" id="Informasi<?php echo $infor++; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-scrollable">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Informasi Kecamatan Weleri</h5>
                      </div>
                      <div class="modal-body">
                      <img src="dashboard/image/<?php echo $row['foto']; ?>" class="card-img-top" alt="Informasi">
                      <p class="card-text fw-bold mt-3"><?= $row['judul_informasi']; ?></p>
                      <p class="card-text text-dark mt-3"><?php echo date('d-m-Y', strtotime($row['tanggal'])); ?></p>
                      <p class="card-text text-primary"><?= $row['isi_informasi']; ?></p>
                      </div>
                      <div class="modal-footer">
                       <button type="button" class="btn btn-success d-flex align-items-center" data-bs-dismiss="modal"><ion-icon name="arrow-undo"></ion-icon> Kembali</button>
                      </div>
                    </div>
                  </div>
                </div>
